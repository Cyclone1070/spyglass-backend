...
namespace spyglass_backend.Features.Search
{
    ...
    )
    {
        ...
        public async IAsyncEnumerable<ResultDto> GetOrStartSearchStream(
            string query,
        ...
        )
        {
            ...
            var searchStream = _activeSearches.GetOrAdd(
                normalisedQuery,
                q =>
                {
        ...
        private async Task StartSearchInBackground(
        ...
        )
        {
            ...
            try
            {
                ...
                if (cachedResult != null)
                {
                    _logger.LogInformation(
                        "Found cached results in MongoDB for query: {Query}",
                        normalisedQuery
                    );
                ...
                var links = await mongoLinkService.GetAsync(
                    "responseTime",
                    SortDirection.Ascending
                );
                var results = new List<Result>();
                await foreach (
                    var result in _searchService.SearchLinksAsync(normalisedQuery, links)
                )
                {
                    results.Add(result);
                    var resultDto = ToResultDto(result);
            ...
            finally
            {
                ...
                _ = Task.Run(async () =>
                {
                    ...
                    {
                        _logger.LogInformation(
                            "Cleaned up completed search stream for query: {Query}",
                            normalisedQuery
                        );
                    }
                });
