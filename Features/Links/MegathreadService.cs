...
namespace spyglass_backend.Features.Links
{
		...
		SearchLinkService searchLinkService)
	{
		...
		public async Task<List<Link>> ScrapeMegathreadAsync()
		{
			...
			_logger.LogInformation("First pass: Attempting to scrape {Count} links normally...", websiteLinks.Count);
			await Parallel.ForEachAsync(websiteLinks, parallelOptions, async (websiteLink, _) =>
			{
				...
				{
					_logger.LogWarning("Got 403 for {Url}. Routing to proxy queue.", websiteLink.Url);
					failedLinks.Add(websiteLink);
				}
				catch (Exception e)
				{
					_logger.LogError(e, "An error occurred during first pass for {Url}", websiteLink.Url);
				}
			});
			...
			{
				_logger.LogInformation("Second pass: Retrying {Count} forbidden links through proxy...", failedLinks.Count);
				var proxyOptions = new ParallelOptions
				{
					MaxDegreeOfParallelism = 40 // Concurrency limit for proxy
				};

				await Parallel.ForEachAsync(failedLinks, proxyOptions, async (websiteLink, _) =>
				{
					try
					{
						await ProcessWebsiteLinkAsync(websiteLink, true, searchLinks, finalLinks);
					}
					catch (Exception e)
					{
						_logger.LogError(e, "Final failure for {Url} even through proxy.", websiteLink.Url);
					}
				});
			}
		...
		private async Task ProcessWebsiteLinkAsync(WebsiteLink websiteLink, bool useProxy, ConcurrentBag<SearchLink> searchLinks, ConcurrentBag<Link> finalLinks)
		{
			_logger.LogDebug("Processing website link: {Url} (Proxy: {UseProxy})", websiteLink.Url, useProxy);
			...
			var (noResultDoc, noResultResponseTime) = await _webService.GetHtmlDocumentAsync(noResultUrl, new Uri(websiteLink.Url), useProxy);
			...
			var (withResultsDoc1, withResultsResponseTime1) = await _webService.GetHtmlDocumentAsync(string.Format(searchLink.SearchUrl, HttpUtility.UrlEncode(queries[0])), new Uri(websiteLink.Url), useProxy);
			var (withResultsDoc2, withResultsResponseTime2) = await _webService.GetHtmlDocumentAsync(string.Format(searchLink.SearchUrl, HttpUtility.UrlEncode(queries[1])), new Uri(websiteLink.Url), useProxy);
			...
			_logger.LogInformation("Successfully processed {Url} (Proxy: {UseProxy})", websiteLink.Url, useProxy);
		}
