...
namespace spyglass_backend.Features.Links
{
    ...
    {
        private readonly IMongoCollection<Link> _linksCollection = database.GetCollection<Link>(
            "links"
        );
        ...
        )
        {
            ...
            return await _linksCollection.Find(_ => true).Sort(sortDefinition).ToListAsync();
        }
        ...
        public async Task RemoveAllAsync() => await _linksCollection.DeleteManyAsync(_ => true);
    }
}
